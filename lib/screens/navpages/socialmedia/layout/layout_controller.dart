...
import 'package:flutter_application_2023/screens/navpages/socialmedia/modules/chats/chat_screen.dart';
import 'package:flutter_application_2023/screens/navpages/socialmedia/modules/feeds/feeds_screen.dart';
import 'package:flutter_application_2023/screens/navpages/socialmedia/modules/my_account/my_account_screen.dart';
...
import 'package:flutter_application_2023/screens/navpages/socialmedia/shared/network/remote/diohelper.dart';
...
// NOTE: -------------------Bottom Navigation------------------------
  ...
  final appbar_title = ['Social Media', 'Chat', 'Notifications', 'Account'];
  ...
  Future<void> pickPostImage() async {
    ...
    if (pickedFile != null) {
      ...
      await decodeImageFromList(File(_postimage!.path).readAsBytesSync())
          .then((value) {
        print("image width : ${value.width}");
        print("image height : ${value.height}");
...
// NOTE -------------------------- Old Get All Posts------------------------
  ...
  void likePost(String postId, {bool isForremove = false}) {
    ...
    FirebaseFirestore.instance
        ...
        .get()
        .then((doc_of_post) {
      if (doc_of_post.docs.isNotEmpty) {
        if (!isForremove) {
  ...
  Future<void> getAllUsers() async {
    ...
    await FirebaseFirestore.instance.collection('users').get().then((value) {
      for (var usermodel in value.docs) {
        ...
        if (value.docs.isNotEmpty) {
          ...
          if (listOfMyChatIds.isNotEmpty) {
            userRef
              ...
              if (value.docs.isNotEmpty) {
                for (var doc_of_user in value.docs) {
              ...
              for (var element in listOfMyChatIds) {
                print("id :$element");
              }
              for (var element in myfriendsMesage) {
                if (element.senderId == uId) {
                  print('friendMessageId :${element.receiverId}');
                } else {
                  print("friendMessageId :${element.senderId}");
                }
              }
...
// NOTE push notification when a friend add a new post
  void pushNotification() {
    ...
    DioHelper.postData(url: 'https://fcm.googleapis.com/fcm/send', data: {
      ...
      "notification": {
        "body": "see details",
        "title": "${_socialUserModel!.name} Add new Post",
        "sound": "default"
      },
      "android": {
        "priortiy": "HIGH",
  ...
  void pushNotificationOnLike() {
    DioHelper.postData(url: 'https://fcm.googleapis.com/fcm/send', data: {
      ...
      "notification": {
        "body": "see details",
        "title": "${_socialUserModel!.name} Like Your post",
        "sound": "default"
      },
      "android": {
        "priortiy": "HIGH",
...
//
  ...
  Future<void> getStories() async {
    ...
    await FirebaseFirestore.instance
      ...
      for (var doc_of_stories in querySnap_of_stories.docs) {
